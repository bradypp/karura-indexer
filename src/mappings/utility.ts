...
async function getLiquidityPool(
  ...
  if (storage.isV1008) {
    const balances = storage.getAsV1008(
      key as [v1008.CurrencyId, v1008.CurrencyId]
    )
    return balances
  }
  if (storage.isV1009) {
    const balances = storage.getAsV1009(
      key as [v1009.CurrencyId, v1009.CurrencyId]
    )
    return balances
  }
  if (storage.isV1019) {
    const balances = storage.getAsV1019(
      key as [v1019.CurrencyId, v1019.CurrencyId]
    )
    return balances
  }
  if (storage.isV2001) {
    const balances = storage.getAsV2001(
      key as [v2001.CurrencyId, v2001.CurrencyId]
    )
    return balances
  }
  if (storage.isV2010) {
    const balances = storage.getAsV2010(
      key as [v2001.CurrencyId, v2001.CurrencyId]
    )
    return balances
  }
  if (storage.isV2011) {
    const balances = storage.getAsV2011(
      key as [v2011.CurrencyId, v2011.CurrencyId]
    )
    return balances
  }
  if (storage.isV2022) {
    const balances = storage.getAsV2022(
      key as [v2022.CurrencyId, v2022.CurrencyId]
    )
    return balances
  }
  const balances = storage.getAsV2041(
    key as [v2041.CurrencyId, v2041.CurrencyId]
  )
  return balances
}
...
export async function addLiquidityChange(
  ...
  if (initial == null) {
    let [b0, b1]: [bigint, bigint] = await getLiquidityPool(ctx, [
      currency0,
      currency1,
    ])
