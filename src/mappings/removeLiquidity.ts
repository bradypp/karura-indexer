...
async function getRemoveLiquidityParams(
  ctx: EventHandlerContext
): Promise<RemoveLiquidityParams> {
  const event = new DexRemoveLiquidityEvent(ctx)
  if (event.isV1008) {
    const [who, currency0, pool0, currency1, pool1, shareDecrement] =
      event.asV1008
...
export async function handleRemoveLiquidity(
  ...
  let { cur0, pool0, cur1, pool1 } = await getRemoveLiquidityParams(ctx)
