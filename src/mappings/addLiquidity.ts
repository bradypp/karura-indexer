...
async function getAddLiquidityParams(
  ctx: EventHandlerContext
): Promise<AddLiquidityParams> {
  const event = new DexAddLiquidityEvent(ctx)

  if (event.isV1008) {
    const [who, currency0, pool0, currency1, pool1, shareIncrement] =
      event.asV1008
...
export async function handleAddLiquidity(
  ...
  let { cur0, pool0, cur1, pool1 } = await getAddLiquidityParams(ctx)
